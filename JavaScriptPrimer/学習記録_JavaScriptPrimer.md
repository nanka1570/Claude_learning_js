...
JavaScriptには非同期処理を行うためのPromiseというビルトインオブジェクト、Async Functionと呼ばれる構文的なサポートがある

<!-- 2025-12-01 -->
### 同期処理
順番に処理していき、一つの処理が終わるまで次の処理は行わない
- JavaScriptは基本的にブラウザのメインスレッド（UIスレッド）で実行されるため、
同期的にブロックする処理があると表示が更新されなかったり、スクロールが効かないということがある

### 非同期処理
非同期処理が終わるのを待たずに次の処理を評価する

<!-- 2025-12-07 -->
#### 非同期処理はメインスレッドで実行される
JavaScriptにおける非同期処理の大部分はメインスレッドで実行される
- 非同期処理が並行処理（Concurrent）として扱われる
- メインスレッドと別のスレッドで実行できるAPIは、並列処理（Parallel）できる

#### 非同期処理と例外処理
非同期処理では、`try...catch`構文を使っても非同期的に発生した例外をキャッチできない
- tryブロックはそのブロック内で発生した例外をキャッチする構文のため

### [ES2015]Promise
Promiseは非同期処理の状態や結果を表現するビルトインオブジェクト

#### `Promise`インスタンスの作成
- `Promise.prototype.then`メソッド
    第一引数にはresolve（成功）時に呼ばれるコールバック関数、第二引数にはreject（失敗）時に呼ばれるコールバック関数を渡す
```Js
// `Promise`インスタンスを作成
...
thenメソッドで失敗時エラー処理をするより、catchメソッドで失敗時エラー処理をしたほうが分かりやすい
```Js
...
#### Promiseの状態
`Promise`インスタンスには、内部的に次の３つの状態が存在する
- **Fulfilled**
  - `resolve`（成功）した時の状態
  - `onFulfilled`が呼ばれる
- **Rejected**
  - `reject`（失敗）または例外が発生した時の状態
  - `onRejected`が呼ばれる
- **Pending**
...
<!-- 2025-12-08 -->
##### Promise.resolve
Fulfilledの状態となったPromiseインスタンスを作成する
- new Promiseの糖衣構文(シンタックスシュガー)
    同じ意味の処理をもとの構文よりシンプルに書ける別の書き方のこと

##### Promise.reject
Rejectedの状態となったPromiseインスタンスを作成する
- 同じくnew Promiseのシンタックスシュガー


#### Promiseチェーン
メソッドチェーンのようにPromiseではthenメソッドの返り値であるPromiseインスタンスにthenめそっでで処理を登録できる
- Promiseが失敗しない限り、順番にthenメソッドで登録した成功時のコールバック関数を呼び出す

<!-- 2025-12-09 -->
##### Promiseチェーンで値を返す
thenやcatchメソッドのコールバック関数は、数値、文字列、オブジェクトなどの任意の値を返せる

##### コールバック関数で`Promise`インスタンスを返す
コールバック関数で`Promise`インスタンスを返した場合は例外的に、同じ状態を持つ`Promise`インスタンスが`then`や`catch`メソッドの返り値となる
- 次のコードではcatchメソッドでログを出力しつつPromise.reject静的メソッドを使ってRejectedなPromiseインスタンスを返している。
これによって、mainで発生したエラーのログを取りながら、Promiseチェーンはエラーのまま処理を継続できる。
```Js
function main() {
...
Promiseの`finally`メソッドは成功時、失敗時どちらの場合でも呼び出されるコールバック関数を登録できる
- `try...catch...finally`構文の`finally`節と同様の役割を持つメソッド
```Js
...
##### Promiseチェーンで逐次処理
Promiseチェーンで非同期処理の流れを書くメリットの一つとして、複数の非同期処理を順番に処理していく逐次処理がある
```Js
...
`Promise.all`で複数のPromiseを使った非同期処理をひとつのPromiseとして扱える
- `Promise.all`静的メソッドは`Promise`インスタンスの配列を受け取り、新しい`Promise`インスタンスを返す
- 受け取る配列がすべて**Fulfilled**となった場合は、返り値の`Promise`インスタンスも**Fulfilled**となる
- 受け取る配列が一つでも**Rejected**となった場合は、返り値の`Promise`インスタンスも**Rejected**となる

##### 使い方
- ResourceAを取得し終わってからResourceBを取得という逐次的な処理は、Promiseチェーンでリソースを取得する
- ResourceAとBどちらを先に取得しても問題ない場合は、`Promise.all`静的メソッドを使う

##### Promise.race
`Promise.race`静的メソッドでは複数のPromiseを受け取り、Promiseが1つでも完了した（Settled状態となった）時点で次の処理を実行する
**=>**`Promise.race`に対し`Promise.all`静的メソッドは、複数のPromiseがすべて完了するまで待つ処理
- 配列の名kで一番最初に**Settled**となったPromiseが**Fulfilled**の場合は、新しい`Promise`インスタンスも**Fulfilled**になる
- **Rejected**の場合は、新しい`Promise`インスタンスも**Rejected**になる

<!-- 2025-12-10 -->
### [ES2017] Async Function
Async Functionは必ず`Promise`インスタンスを返す関数を定義する構文
- 関数の前に`async`を付ける
```Js
...
1. Async Functionが価をreturnした場合、その返り値を持つ**Fulfilled**なPromiseを返す
2. Promiseをreturnした場合、その返り値のPromiseをそのまま返す
3. 例外が発生した場合は、そのエラーを持つ**Rejected**なPromiseを返す

### `await`式
非同期処理が同期処理のように上から下へと順番に実行するような処理順で書ける
- 右辺の`Promise`インすガンスが**Fulfilled**または**Rejected**になるまでその場で非同期処理の完了を待ち、`Promise`インスタンスの状態が変わると次の行の処理を再開する
- await式は次の個所でしか利用できず、ほかの個所では構文エラーになる
  - Async Functionの関数の直下
  - ECMAScriptモジュールの直下

- 右辺のPromiseが**Fulfilled**なら、resolveされた値が返り値になる
- 右辺のPromiseが**Rejected**なら、その場でエラーを`throw`する

#### Promiseチェーンを`await`式で表現する
`await`式を使ってリソースが取得できるまで待ち、その結果を変数を`results`に追加していく形で逐次処理をしたもの
```Js
function dummyFetch(path) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
...
他の構文やPromise APIと組み合わせたAsync Function

#### Async Functionと反復処理
複数の非同期処理を行う際に、forループなどの反復処理と組み合わせることができる

#### `await`式はAsync Functionの直下でのみ利用可能
Async Functionで`await`式を利用する際は、`await`式が関数の直下でのみ利用可能

- Async Functionとコールバック関数を組み合わせた場合
  - コールバック関数を使わずにすむforループと`await`式を組み合わせる
  - 複数の非同期処理を１つのPromiseにまとめることで`await`式を使わないようにする方法がある

#### [ES2022] Module直下での`await`式
Moduleの直下ではAsync Functionで囲まなくても`await`式が利用できる
=>Top-Level`await`

#### [コラム] エラーファーストコールバック
非同期処理におけるコールバック関数の呼び出し方を決めた次のルール
- 処理が失敗した場合は、コールバック関数の１番目の引数にエラーオブジェクトを渡して呼び出す
- 処理が静甲した場合は、コールバック関数の１番目の引数には`null`を渡し、２番目以降の引数に成功時の結果を渡して呼び出す
